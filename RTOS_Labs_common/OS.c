...
void stack_init(){
	for(int i=0; i<NUM_THREADS; i++){
		...
		(threads[i].next) = &threads[(i+1)%NUM_THREADS];    //Initialize next pointer to next stack block in the contiguous allocation
...
//			periodic_threads[i].current_count -= CURRENT_CONTEXT_SWITCH_PERIOD;
//			if(periodic_threads[i].current_count==0){
//				(*(periodic_threads[i].periodic_function))();
//				periodic_threads[i].current_count = periodic_threads[i].period;
//				
//			}
//		}
//	}
	//UART_OutString("systick\n\r");
	//PF2 = 0x0F;
...
uint32_t OS_Id(void){
  ...
  return Runptr->id; // replace this line with solution
};
...
uint32_t OS_MsTime(void){
  // put Lab 1 solution here
  return Counts; // replace this line with solution
};
