...
int32_t Heap_Stats(heap_stats_t *stats){
  ...
  while(inHeapRange(blockStart)){
    if(blockUsed(blockStart)){
      stat.used += blockRoom(blockStart);
      //stat.blocksUsed++;
    }
    else{
      stat.free += blockRoom(blockStart);
      //stat.blocksUnused++;
    }
    blockStart = nextBlockHeader(blockStart);
  }
	stat.free = stat.free*4;
	stat.used = stat.used*4;
	*stats = stat;
	return 0;
 // stat.wordsOverhead = HEAP_SIZE_WORDS - stat.wordsAllocated - stat.wordsAvailable;
  //return stat;
}
