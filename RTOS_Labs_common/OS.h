...
#define TIME_HALF_MS                  40000
#define TIME_2MS    									(2*TIME_1MS)  
#define TIME_500US  									(TIME_1MS/2)  
#define TIME_250US  									(TIME_1MS/5)  
#define CURRENT_CONTEXT_SWITCH_PERIOD  2*TIME_1MS
#define CURRENT_CONTEXT_SWITCH_PERIOD_USECS 2000
...
//typedef struct periodic_thread periodic_thread;
struct TCB{
	int32_t* sp;
	struct TCB* next;
	uint8_t status;
	uint16_t id;
	int32_t sleep_time_usecs;
	//uint32_t priority;
	uint32_t WorkingPriority;   //Priority used by scheduler (could be different from original due to aging priority boosting)
	uint32_t FixedPriority;     //Original priority
	uint32_t Age;
	Sema4Type* block_semaphore;
	struct TCB* children[5];
	struct TCB* parent;
	uint8_t num_children;
	uint8_t is_process_root_thread;
	uint8_t process_creator;
	PCB* process_created;
	PCB* root_process;
	
	//struct periodic_thread* periodic;
...
typedef struct periodic_thread PThread;
enum thread_state{
	RUNNING,
	FREE,
	BLOCKED,
...
#define THREAD_RUNNING_ACTIVE 1
#define THREAD_RUNNING_INACTIVE 2
